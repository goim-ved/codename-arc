...
🚧 Garage stage (v0.1-in-progress). Current one-line truth: Milestone 8 complete (`arc solve <case-file>` CLI with polar Newton-Raphson AC, lossless linear DC, sparse Markowitz LU, dense Gaussian elimination, JSON output formatting, and comprehensive integration tests; 52 passing tests); Milestone 9 (determinism verification & performance baseline) ready to start.
...
# Run full test suite across workspace (52 passing tests)
cargo test --workspace

# Run automated numerical regression harness across all 6 IEEE benchmark cases (12 evaluations)
cargo run --bin arc -- test

# Solve AC power flow on an IEEE benchmark case using the sparse solver
cargo run --bin arc -- solve data/cases/case14.m

# Solve DC power flow and output machine-readable JSON
cargo run --bin arc -- solve data/cases/case14.m --mode dc --format json
...
- **M8 — CLI & Ergonomics**: Full `arc solve <case-file>` interface, tabular/JSON formatting, parameter controls. *(Done)*
- **M9 — Determinism & Benchmark Baseline**: Bit-diff testing in CI and Criterion performance baseline. *(Next)*
